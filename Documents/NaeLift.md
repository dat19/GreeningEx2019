...
- 移動する際、移動予定座標を算出
- 苗を置く場所を、単位ごとに丸め込んで決定
- 苗を置きたい場所にあるものを列挙
  - 床が見つかったら移動をキャンセルする

移動を継続するか、キャンセルするかして、座標を確定したあとに、以下を実行する。

- 苗を置きたい場所が変わっていたら、苗を置きたい場所を再調査
- 何かオブジェクトがあったら置けないので、置く先のオブジェクトを消して、調査終わり
- 苗の直下に床があるかを調査
  - 床があれば、そこに半透明の苗を表示
  - 床がなければ置けないので、苗を消す
...
以上で、苗を置く候補のX座標が算出できる。




- 算出したステラからの距離より遠くには置かない
